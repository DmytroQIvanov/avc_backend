...
export class AdminService {
    ...
    async loginAdmin(login:string, password:string){
        ...
        if(admin && admin.password == password){
            ...
            const accesToken = this.jwtService.sign({login},{expiresIn:"3600s"});

            const randomKey = uuid()
            const accesSlice = accesToken.slice(-8)
            const refreshToken = this.jwtService.sign({accesSlice,randomKey},{expiresIn:"9600h",secret:accesSlice});

            return {accesToken, refreshToken,login}
        }
        throw new HttpException('Password or login doe`s match ',HttpStatus.FORBIDDEN)
    ...
    async checkToken(accesToken){
        ...
        const admin =await this.adminRepository.findOne({login:accesTokenObj["login"]})
        ...
        if(admin){
            ...
            throw new HttpException("Token is not valid CheckToken",HttpStatus.FORBIDDEN)
        }
      }
    async addAdmin(code:string,{firstName,lastName,login,password}){
        if(code !="sT7PZ7Tmn99L94cr") throw new HttpException("code",HttpStatus.FORBIDDEN)
        if(!login || !password || !firstName || !lastName) throw new HttpException("Some field is missing",HttpStatus.BAD_REQUEST)
        const admin = await this.adminRepository.create({firstName,lastName,login,password})
